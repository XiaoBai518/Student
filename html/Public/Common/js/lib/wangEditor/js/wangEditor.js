    ...
    Menu.fn.changeSelectedState = function (state) {
        ...
        if (state != null && typeof state === 'boolean') {
            ...
            if (state) {
                // 选中
                self.$domNormal.hide();
                self.$domSelected.show().removeAttr('style');//.removeAttr ie bug
            } else {
                // 未选中
                self.$domNormal.show();
...
_e(function (E, $) {
    ...
    Txt.fn.bindEnterForDiv = function () {
        ...
        function divHandler() {
            ...
            var style=$keydownDivElem.attr('style');
            var $pElem;
            if(style){
                $pElem = $('<p style="'+style+'">' + $keydownDivElem.html() + '</p>');
            }else{
                $pElem = $('<p>' + $keydownDivElem.html() + '</p>');
            }
        ...
        $txt.on('keydown keyup', function (e) {
            ...
            if (!targetElem) {
                ...
                if (e.type === 'keyup') {
                    ...
                    var style=$targetElem.attr('style');
                    if(style){
                        $pElem = $('<p style="'+style+'">' + $targetElem.html() + '</p>');
                    }else{
    ...
    Txt.fn.bindPasteFilter = function () {
        ...
        $txt.on('paste', function (e) {
            var currentNodeName = editor.getRangeElem()&&editor.getRangeElem().nodeName;
            ...
            if (!editor.config.pasteFilter) {
                // 配置中取消了粘贴过滤
            	editor.command(e, 'insertHtml', orginHtml);
        ...
        function getResult(elem) {
            ...
            if (['blockquote'].indexOf(nodeName) >= 0) {
                ...
                htmlForP = htmlForP.replace(/<.*?>/ig, function (tag) {
                    if (tag === '</u>' || tag.indexOf('<u ') === 0||tag === '</a>' || tag.indexOf('<a ') === 0 || tag.indexOf('<img ') === 0) {
                        return tag;
                    } else {
                        return '';
                    }
        ...
        function removeAttrs(elem) {
            ...
            var attrNames = [];
            var exception = ['href', 'target', 'src','rowspan', 'colspan','class','data-latex']; //例外情况
...
_e(function (E, $) {
    ...
    function loadHljs() {
        ...
        if(document.getElementById('highlight'))return;
        var script = document.createElement("script");
        script.type = "text/javascript";
        script.id = "highlight";
    ...
    E.createMenu(function (check) {
        ...
        function contentHandle($content) {
            ...
            $btnSubmit.click(function (e) {
                ...
                if (!menu.selected) {
                    // 拼接html
                    var html = '<br>'+codeTpl.replace('{#langClass}', langClass).replace('{#content}', val)+'<br>';
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            var resultText = JSON.parse(resultText).src;
            ...
            if (resultText.indexOf('error|') === 0) {
                ...
                img.onload = function () {
                    ...
                    E.log('已插入图片， 地址 by plugin:' + resultText);
                    img = null;
                };
                img.onerror = function () {
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        editor.xhrUploadImg = function (opt) {
            ...
            var fileType = opt.fileType || 'image/png';  // 无扩展名则默认使用png
            ...
            xhr.onload = function () {
                ...
                if(opt.selfsuccess){
                	opt.selfsuccess(xhr.responseText);
                }else{
...
_e(function (E, $) {
	...
	// 渲染事件
        ...
        function render() {
            ...
            isRender = true;
            if(menuContainer.$menuContainer.is(':hidden')){
                menuHeight = 0;
            }
            $progress.css({
                top: menuHeight + 'px'
...
_e(function (E, $) {
	...
	if (!uploadImgUrl) {
            ...
            $.each($imgs, function () {
                ...
                if (reg.test(base64)) {
                    ...
                    editor.xhrUploadImg({
                        ...
                        fileType: type,
                        name: uploadFileName,
                        selfsuccess:function(tData){
                    		var src = JSON.parse(tData).src;
                    		$img.attr('src',src);
                    	}
                    });
                } else {
                	var regt = /^file:/;
                	if(regt.test(base64)){//读取全部文件
                		E.log('src 为 ' + base64 + ' ，是file:\\类型的路径，尝试处理...');	
                        ...
                        img.onerror = function() {
                        	$img.attr('title',"直接从word中复制数学公式粘贴，会导致公式无法正常显示，用360浏览器“兼容模式”下，可以上传");
                        	$img.attr('width','200px');
                        	$img.attr('height','100px');
                        };
                	}else{
                        E.log('src 为 ' + base64 + '暂时不支持上传');	
                	}
                }
            });
        }
        ...
        $txt.on('paste', function (e) {
            pasteEvent = e;
            for(ts in e.originalEvent){
                console.warn(ts+':'+e['originalEvent'][ts]);
            }
            // console.info(e);
	...
	// 说明：设置了 max-height 之后，$txt.parent() 负责滚动处理
        ...
        function bindDragEvent() {
            ...
            function mousemove (e) {
                ...
                // 计算差额
                diffX = e.pageX - _x;
                diffY = diffX/scale;//默认比例缩放，ctrl自由缩放
                if(e.shiftKey==1){
                    //shift被按下
                ...
                // console.log(e.pageY+" "+_x+" "+diffX+" "+e.pageY+" "+_y+" "+diffY+" "+dragMarginLeft+" "+dragMarginTop+" "+editor.$txt.width())
                

                // --------- 计算图片的大小 ---------
                var currentImgWidth = imgWidth + diffX;
                var currentImggHeight = imgHeight + diffY;
                if(currentImgWidth>=editor.$txt.width()||currentImgWidth<=70){
                    return;
                }
        ...
        $currentTxt.on('mousedown', 'img', function (e) {
            e.preventDefault();
        }).on('click', 'img:not(".kfformula")', function (e) {
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var menu = new E.Menu({
            ...
            // 正常状态和选中装下的dom对象，样式需要自定义
            $domNormal: $('<a href="javascript:;" tabindex="-1"><i class="wangeditor-menu-file-omega"></i></a>'),
            $domSelected: $('<a href="javascript:;" tabindex="-1" class="selected"><i class="wangeditor-menu-file-omega"></i></a>')
        });
        // 菜单正常状态下，点击将触发该事件
        menu.clickEvent = function (e) {}
        editor.menus[menuId] = menu;
...
/*_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        // 要插入的符号（可自行添加）
        var symbols = ['单选',]
        var radiohtml = '<div class="white">\
            <div class="ques-title">\
                <div class="title-txt" contenteditable="true" placeholder="请在这里粘贴〖题干〗和〖选项〗，系统会自动分离"></div>\
            </div>\
            <div class="choose-item choose radio">\
                <div class="item">\
                    <div class="tips">\
                        <span>设为答案</span>\
                        <i></i>\
                    ...
                    <span>A</span>\
                    <div class="item-txt" contenteditable="true" placeholder="选项..."></div>\
                    <a href="javascript:;" class="del-item"></a>\
                </div>\
                <div class="item">\
                    <div class="tips">\
                        <span>设为答案</span>\
        ...
        $container.on('click', 'a', function (e) {
            var $a = $(e.currentTarget);
            var s = '<div class="s" style="border:1px solid #dcdcdc"><span>a.</span><p contenteditable="true">123</p></div>';

            // 执行插入的命令
            editor.command(e, 'insertHtml', radiohtml);
            editor.$txt.removeAttr('contenteditable');
            editor.$txt.html($(radiohtml));
            editor.$txt.find('[contenteditable=true]').click(function(){
                editor.$txt = $(this);
                editor.txt.init();
            })
        });
