...
import com.student.hbnu.util.SpringContextUtil;

import io.netty.handler.codec.http.HttpHeaders;



@ServerEndpoint(path="/faceCollection",port=8081,maxFramePayloadLength=1000000)
@Component
public class FaceCollectionWebSocket {
	//日志
    ...
    protected static CopyOnWriteArraySet<FaceCollectionWebSocket> webSocketSet = new CopyOnWriteArraySet<FaceCollectionWebSocket>();
...
//  //加载脸部分类器
    ...
    public FaceCollectionWebSocket() {
        log.info("面部信息接收  init...");
    }
    ...
    public void onBinary(Session session, byte[] message) throws IOException {
    	//计数器
    	 ...
    	 if(count>4) {
    		 ...
    		 train = (Train) SpringContextUtil.getApplicationContext().getBean(Train.class);
    		 train.incrementalTrain(uid);
    	        ...
    	        //直方图均衡化，图像增强，暗的变亮，亮的变暗。
    	         ...
    	         if(faceDetections.toArray().length==0) {
    	             ...
    	             faceROI = new Mat(image, faceDetections.toArray()[0]);
    	              ...
    	              Imgcodecs.imwrite("F:\\faceCollection\\"+
    	            		  								uid+
    	            		  								"-"+
    	            		  								count+
    	            		  								".jpg", faceROI);
